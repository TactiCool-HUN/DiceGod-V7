...
async def response(message: discord.Message):
	...
	sent = await td.send_message(message, ascii_mapping(fire_map, vision_map))
	for game in bot_state.active_fireball_sweeper_games:
		if game['id'] == game_id:
			game['id'] = sent.id
...
def click_space(action: str, fire_map: list[list[int]], vision_map: list[list[int]], row: int, column: int) -> str:
	if vision_map[row][column] == 0:
		if action == 'flag':
			...
			if fire_map[row][column] == 1:
				pass # game over
			elif neighbour_checker(fire_map, row, column) == 0:
